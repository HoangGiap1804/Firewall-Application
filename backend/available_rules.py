...
def make_rule_key_from_cmd(cmd):
    """
    Chuyển 1 lệnh iptables thành key gọn: <TARGET> <PROT_NUM> -- <SRC> <DEST> [extras]
    Bỏ qua chain (INPUT/OUTPUT/...).
    """
    try:
        # Lấy target (ACCEPT/DROP/LOG...)
        target = cmd[cmd.index("-j") + 1] if "-j" in cmd else "ACCEPT"

        # Gán số giao thức (nếu có)
        ...
        extras = []
        if "--dport" in cmd:
            extras.append(f"--dport {cmd[cmd.index('--dport') + 1]}")
        if "--icmp-type" in cmd:
            extras.append(f"--icmp-type {cmd[cmd.index('--icmp-type') + 1]}")
        if "-m" in cmd:
            extras.append(f"-m {cmd[cmd.index('-m') + 1]}")
...
class AvailableRules(QObject):
    ...
    # Rule groups có thể kèm thông tin nhóm (metadata)
    RULE_GROUPS = {
        "ICMP Flood": {
            "group": "ICMP Flood",
            "rules": [
    ...
    def toggleRule(self, group_name: str, enable: bool) -> str:
        ...
        if enable:
            for cmd in cmds:
                ...
                # Nếu lệnh chạy thành công, lưu group vào rules_meta.json
                if res.returncode == 0:
                    try:
            ...
            # Xoá các rule thuộc group này khỏi metadata
            keys_to_remove = [k for k, v in meta.items() if v == group_meta]
